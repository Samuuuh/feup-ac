...
def init_comp_db(id):
  connection = sqlite3.connect(f'./data/ac-comp_v-{id}.db')
  ...
  contents = csv.reader(open('./data/raw/loan_comp.csv'), delimiter=';')
  ...
  contents = csv.reader(open('./data/raw/trans_comp.csv'), delimiter=';')
...
def init_dev_db(id):
  connection = sqlite3.connect(f'./data/ac-dev_v-{id}.db')
  cursor = connection.cursor()
  ...
  contents = csv.reader(open('./data/raw/card_dev.csv'), delimiter=';')
  insert_records = "INSERT INTO card(card_id, disp_id, type, issued) VALUES(?, ?, ?, ?)"
