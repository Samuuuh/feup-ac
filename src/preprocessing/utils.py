...
def read_csv(file: str, columns: list = None) -> pd.DataFrame: 
    ...
    if columns is None:
        return pd.read_csv(position + '/../data/raw/' + file + '.csv', low_memory=False, sep=";")
    return pd.read_csv(position + '/../data/raw/' + file + '.csv', usecols=columns, low_memory=False, sep=";")


def write_csv(df: pd.DataFrame, file: str, index: bool = True) -> str: 
    position =  os.path.dirname(os.path.abspath(__file__)) + "/.."
    return df.to_csv(position + '/../data/preprocessed/' + file + '.csv', sep=';', index=index) 
